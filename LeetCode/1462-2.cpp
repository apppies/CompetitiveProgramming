...
// unordered_set replaced with a 128bit number, as we get a maximum of 100 nodes
// uint128 crashes, using bitset instead
private:
    vector<bitset<100>> allrequisites;
    vector<bool> checked;

public:
    bitset<100> getPrerequisites(int courseId){
        if (!checked[courseId]) {
            // Never checked before, recursively get requisites from parents
            bitset<100> requisites = 0;
            for (size_t i = 0; i < checked.size(); i++)
            {
                if (allrequisites[courseId].test(i)) {
                    requisites |= getPrerequisites(i);
                }
            }
            
            allrequisites[courseId] |= requisites;
    ...
    vector<bool> checkIfPrerequisite(int numCourses, vector<vector<int>>& prerequisites, vector<vector<int>>& queries) {
        ...
        allrequisites = vector<bitset<100>>(numCourses);
        vector<bool> answer;
        ...
        for (auto const q: queries){
            answer.push_back(allrequisites[q[0]].test(q[1]));
        }
